#!/usr/bin/env python3
"""
Test Daily Email Digest - Sends to jayden@kenaidefense.com only
...
Options:
    --test-email EMAIL    Override test email (default: jayden@kenaidefense.com)
"""
...
TEST_EMAIL = "jayden@kenaidefense.com"
...
def _parse_email_addresses(email_field: str) -> list[str]:
    """Parse email field - returns list but we'll override in test mode"""
...
def _send_email(to_email: str, subject: str, html_body: str, text_body: str = ""):
    ...
    try:
        ...
        logging.info("✅ TEST EMAIL sent to %s via Gmail SMTP", to_email)
    except Exception as e:
...
def _render_email(recipient_email: str, tech_desc: str, picks: pd.DataFrame,
    ...
    if picks.empty:
        ...
        subject = f"🧪 TEST - KIP Daily Digest{tech_label}: No Close Matches Today"
        html = f"""
        {email_wrapper}
          {header}
          <div style="padding: 32px 24px;">
            <div style="background-color: #fef3c7; border-left: 4px solid #f59e0b; padding: 16px 20px; border-radius: 4px; margin: 0 0 24px 0;">
              <p style="color: #92400e; font-size: 14px; font-weight: 600; margin: 0 0 4px 0;">
                🧪 TEST MODE - This is a test email
              </p>
              <p style="color: #92400e; font-size: 13px; margin: 0;">
                Production emails would go to: {recipient_email}
              </p>
            </div>
    ...
    tech_label = f" [{tech_name}]" if tech_name else ""
    subject = f"🧪 TEST - KIP Daily Digest{tech_label}: {len(rows_html)} Top {'Match' if len(rows_html) == 1 else 'Matches'} for {datetime.now().strftime('%B %d, %Y')}"
...
def main():
    global engine, oai, TEST_EMAIL

    # Parse arguments
    parser = argparse.ArgumentParser(description="Test daily digest emails")
    parser.add_argument("--test-email", default="jayden@kenaidefense.com",
    ...
    with engine.connect() as conn:
        ...
        if notices.empty:
            ...
            for _, tech_area in tech_areas.iterrows():
                subject, html = _render_email(
                    ...
                    tech_area["technology_description"],
                    pd.DataFrame(),
                    tech_name=tech_area["technology_name"]
                )
                # Override recipient to test email
                _send_email(TEST_EMAIL, subject, html)
        ...
        for _, tech_area in tech_areas.iterrows():
            ...
            production_email = tech_area["emails"]  # Original production email
            ...
            if not final_matches.empty:
                subject, html = _render_email(
                    production_email,  # Shows original email in test banner
                    desc,
                    final_matches,
                    tech_name=tech_name
                )
                ...
                # Override recipient to test email
                _send_email(TEST_EMAIL, subject, html)
                logging.info(f"✅ Test email sent: {len(final_matches)} matches for {tech_name}")
            else:
                logging.info(f"No matches above threshold for {tech_name}")

    logging.info("=" * 70)
    logging.info("Test digest complete!")
    logging.info(f"All test emails sent to: {TEST_EMAIL}")
    logging.info("=" * 70)
